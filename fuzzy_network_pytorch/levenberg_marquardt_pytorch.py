...
class MSELoss(Loss):
    ...
    def forward(self, y_pred: Tensor, y_true: Tensor) -> Tensor:
        ...
        """
        # Force y_true to be on the same device as y_pred
...
class BoundaryRegularizedMSELoss(nn.Module):
    """
    Computes MSE Loss + Regularization penalty for parameters outside [min, max].
    """
    def __init__(self, model: nn.Module, min_limit: float, max_limit: float, reg_weight: float = 0.1) -> None:
        """
        Args:
            model: The PyTorch model (needed to access parameters).
            min_limit: Lower bound for parameters (e.g., input data min).
            max_limit: Upper bound for parameters (e.g., input data max).
            reg_weight: Lambda factor to weight the regularization vs MSE.
        """
        super().__init__()
        self.model = model
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        # 1. Standard MSE
        mse_loss = (y_pred - y_true).square().mean()

        # 2. Boundary Regularization
        reg_loss = torch.tensor(0.0, device=y_pred.device)
        
        for param in self.model.parameters():
            # Calculate violation: distance below min or above max
            # torch.relu(x) returns x if x>0, else 0.
            
            # Amount param is below min_limit
            under_limit = torch.relu(self.min_limit - param) 
            
            # Amount param is above max_limit
            over_limit = torch.relu(param - self.max_limit)
            
            # Sum of squared violations (L2 style penalty implies smoother gradients)
            reg_loss += torch.sum(under_limit**2 + over_limit**2)

        # Total Loss
        return mse_loss + (self.reg_weight * reg_loss)


    def residuals(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        """
        Computes residuals for LM optimizer. 
        Note: The regularization is primarily applied in the scalar loss.
        """
        return y_pred - y_true
    
    # def residuals(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
    #     """
    #     Computes the complete residual vector for the LM optimizer, 
    #     including prediction residuals and scaled regularization residuals.
    #     """
        
    #     # 1. Prediction Residuals
    #     # Scale by 1/sqrt(N) to ensure sum of squares matches the MSE term.
    #     N = y_pred.numel()
    #     # Ensure y_pred - y_true is a 1D vector before dividing
    #     pred_residuals = (y_pred.flatten() - y_true.flatten()) / (N**0.5)

    #     # 2. Regularization Residuals
    #     reg_residuals_list: List[torch.Tensor] = []
        
    #     # Factor to apply to regularization residuals: sqrt(lambda)
    #     lambda_sqrt = self.reg_weight**0.5 
    ...
    #         # Residuals for lower boundary violation (sqrt(lambda) * violation)
    #         reg_res_under = lambda_sqrt * under_limit
            
    #         # Residuals for upper boundary violation (sqrt(lambda) * violation)
    #         reg_res_over = lambda_sqrt * over_limit
            
    #         # Append flattened residuals for this parameter
    #         reg_residuals_list.append(reg_res_under.flatten())
    #         reg_residuals_list.append(reg_res_over.flatten())

    #     # Concatenate all residuals into a single 1D vector
    #     if reg_residuals_list:
    #         all_residuals = torch.cat([pred_residuals] + reg_residuals_list)
    #     else:
    #         all_residuals = pred_residuals

    #     # Critical for matrix multiplication: 
    #     # Return an (R x 1) column vector. The LM library should accept this shape.
    #     return all_residuals.reshape(-1, 1)
